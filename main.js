 ...
 const iniciarJuego = () => { 
  ...
  cartasJuego.forEach((carta, index) => {
    if (index === 0) active = 'active'
      ...
      carousel.innerHTML += `<div class="carousel-item ${active}">${jugadores[0].nombre}<img src="${carta.image}" class="d-block w-100" alt="...">
...
if(cartasJuego.slice(0,3)){
  cartasj.innerHTML += `<h3>Cartas de  ${jugadores[0].nombre} </h3>
`}else{
  cartasj.innerHTML += `<h3>Cartas de  ${jugadores[1].nombre} </h3>`
} 
...
cj1.innerHTML = cartasJuego.image.slice(0,3)
}
mostrarImagenes()
